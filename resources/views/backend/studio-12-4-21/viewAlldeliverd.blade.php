...
@section('content')
    <div class="right_col" role="main">
        <div class="">
            ...
            <br>
            <div class="row">
                            <form action="{{route('get.order.deliverd')}}" method="post">
                                {{csrf_field()}}
                                <div class="col-md-3">
                                ...
                                <div class="col-md-3">
                                    <button name="" class="btn btn-info">All Deliverd Order</button>
                                </div>
                            </form>
                        </div>
                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_content" style="overflow: scroll;">
                            <table style="border: 1px solid black" class="table table-striped table-bordered table-hover" id="categorytable">
                                ...
                                <tbody>
                                <?php $i=1;  ?>
                               @foreach($alldeliverd as $pc)
                                    <tr>
                                        <th> {{$i++}}</th>
                                        <td>{{$pc->client_name}} </td>
                                        <td>{{$pc->customer_id}}</td>
                                       <!-- <td><a href="{{route('update.order.deliverd',$pc->service_id)}}" class="btn btn-primary" color="FF00CC" >{{$pc->service_id}}</a></td>-->
                                       <td>{{$pc->service_id}}</td>
                                       <td><a href="{{route('upload.order.photos',$pc->service_id)}}" class="btn btn-primary" color="FF00CC" >Photo Upload</a></td>
                                        <td>{{$pc->phone}} </td>
                                        <td>{{$pc->email}} </td>
                                        <td>{{$pc->address}} </td>
                                        <td>{{$pc->subtotal}} </td>
