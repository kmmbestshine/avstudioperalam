...
@section('content')
    <div class="right_col" role="main">
        <div class="">
            ...
            <br>
            <div class="row">
                            <form action="{{route('esevai.undeliverd.reportlist')}}" method="post">
                                {{csrf_field()}}
                                <div class="col-md-3">
                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_content" style="overflow: scroll;">
                            <table style="border: 1px solid black" class="table table-striped table-bordered table-hover" id="categorytable">
                                ...
                                <tbody>
                                <?php $i=1;  ?>
                               @foreach($undeliverdreport as $pc)
                                    <tr>
                                        <th> {{$i++}}</th>
                                        <td>{{$pc->client_name}} </td>
                                        <td><a href="{{url('update-esevai-undeliverd',$pc->customer_id)}}" class="btn btn-primary" color="FF00CC" >{{$pc->customer_id}}</a></td>
                                        <td>{{$pc->invoice_no}}</td>
                                        <td>{{$pc->phone}} </td>
                                        <td>{{$pc->email}} </td>
                                        <td>{{$pc->address}} </td>
